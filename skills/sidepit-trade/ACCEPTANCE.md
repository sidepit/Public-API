...
This is the release gate for the pair: `sidepit-onboarding` onboards the human and
...
4. With no credentials, run each documented Bash/protoc/nngcat 12125 recipe
   with its `timeout` wrapper.
   Confirm ACTIVE_PRODUCT, POSITIONS, QUOTE, HISTORICAL_BARS, and SCHEDULES each
   decode a `ReplyRequest`; every nngcat invocation includes `--raw`; each
   request contains exactly one TypeMask; and none uses SNAPSHOT, `account_tx`,
   signing, or a write-capable socket. Also prove that omitting `--raw` exits 0
   with no decoded output, so the documented trap matches the shipped tool.
   Confirm `set -o pipefail` makes an encode, connection, timeout, or nngcat
   failure nonzero instead of letting the final decoder hide it.
5. Run `sidepit_agent.py --help` and every subcommand's `--help`. Confirm no
   command requests or displays an account key.
6. Exercise delegate mint only in an isolated test key directory. Confirm one
   new 0600 write-once file, public output only, and no existing key changed.
7. Use fixtures/mocks for preview and send gates. Confirm an inactive delegate,
   wrong file mode, expired preview, changed position, changed exposure, wrong
   a stale preview, or repeated attempt sends zero transactions. Confirm native
...
ACTIVE truth → status → order choices → preview → the preview → outcome →
cancel/flatten → human web unlock. Confirm every stop-state reports its next command and a copy-paste recovery
...
the agent sending its own preview without any confirmation phrase.

## R1–R15 release checklist

- **R1:** Delegate key recommended; an owner account key also loads in direct mode. The skills teach
...
- **R16:** The keyless direct-read path needs no language runtime or credentials;
  uses Bash, `protoc`, `nngcat`, and a timeout wrapper; preserves
  `set -o pipefail`; includes `--raw` on every request; starts with
  ACTIVE_PRODUCT; names every `ReplyRequestTypes` enum value while explaining
  that the wire field is currently `int32`; covers only one of ACTIVE_PRODUCT,
  POSITIONS, QUOTE, HISTORICAL_BARS, or SCHEDULES per request; and never
  presents SNAPSHOT, combined masks, DELEGATE, UNLOCK, `account_tx`, signing,
  or writes as a one-pipeline recipe.
