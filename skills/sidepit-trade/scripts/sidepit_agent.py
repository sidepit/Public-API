#!/usr/bin/env python3
"""Delegate-only Sidepit lifecycle for an AI agent.
...
def load_delegate(raw_path: str) -> Delegate:
    ...
    if identity.sidepit_id == account:
        raise Stop(
            "this file holds the account key, not a delegate key; do not use it with "
            "an agent. Run the sidepit-locals skill"
        )
...
def cmd_send_order(args) -> int:
    ...
    expected = f"CONFIRM {p['preview_id']}"
    if args.confirm != expected:
        raise Stop(f"confirmation mismatch; expected exactly: {expected}")
...
def cmd_cancel(args) -> int:
    expected = f"CANCEL {args.order_id}"
    if args.confirm != expected:
        raise Stop(f"CANCEL PREVIEW — nothing sent. Confirm exactly: {expected}")
    delegate = load_delegate(args.key_file)
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Delegate-only Sidepit trading with preview and recovery gates")
    ...
    q = sub.add_parser("send-order", help="send only the exact confirmed preview")
    q.add_argument("--key-file", required=True)
    ...
    q = sub.add_parser("send-flatten", help="send only the exact confirmed flatten preview")
    q.add_argument("--key-file", required=True)
    q.add_argument("--preview-file", required=True)
    q.add_argument("--confirm", required=True)
    q.add_argument("--wait-seconds", type=float, default=3.0)
