...
1. [`sidepit-locals`](skills/sidepit-locals/SKILL.md) helps the human enter beta
...
> Sidepit ID first; after it arrives, use LOCK to forward that balance.
...
> `skills/sidepit-locals/SKILL.md`, and onboard me without ever asking for my
...
> preview, but send nothing until I confirm that exact preview.
...
| `skills/sidepit-locals/` | human onboarding: web wallet → deliberate funding → ACTIVE agent |
| `skills/sidepit-trade/` | restricted-agent trading: preview → confirm → order → reconcile → flatten |
